...
use turbo_core::{load_pins, pin_config};
...
fn main() {
    ...
    device_map.emit(&mut std::io::stdout()).expect("Failed to emit device map");

    for m in muxes.values() {
         m.emit(&mut std::io::stdout()).expect("Failed to emit mux");
    }
}
